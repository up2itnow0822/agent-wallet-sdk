...
_Last verified: 2026-05-12 19:00 CT_
...
Use these fields when mapping Stripe Link/SPT, AWS AgentCore/x402, Circle Agent Stack/USDC nanopayments, Google AP2-style, or mobile-wallet delegation flows into agentwallet policy and audit logs.
...
| `wallet_provider` | Identifies who holds/authenticates the payment credential | Link, Stripe Privy, Coinbase CDP, Circle Agent Wallet, on-chain wallet |
...
| `proof` | Verifies authorization/payment completion | x402 proof, SPT grant, signed mandate, Circle Gateway proof, transaction hash |
| `receipt_id` | Auditable post-action evidence | x402 settlement/proof id, Circle Gateway receipt, provider receipt, on-chain tx hash |
...
- **Normalize every provider into the mandate table above.** This keeps Link/SPT, x402, Circle Agent Stack, AP2-style, and mobile-wallet flows comparable for enterprise audits.
...
- Circle Agent Stack, 2026-05-11: https://www.circle.com/blog/introducing-circle-agent-stack-financial-infrastructure-for-the-agentic-economy
